...
public class ConversationMessagesHandler {
    ...
    private ValueEventListener loadMoreEventListener;
...
//        conversationMessagesListeners.add(conversationMessagesListener);
        ...
        if (conversationMessagesChildEventListener == null) {
            ...
            conversationMessagesChildEventListener = conversationMessagesNode.orderByChild(Message.TIMESTAMP_FIELD_KEY)
                    .limitToLast(loadCapacity).addChildEventListener(new ChildEventListener() {
                        @Override
...
//                Log.d(TAG, "observeMessages.onCancelled");
    ...
    class Watcher {
        ...
        public void setFromLoadingMore(boolean fromLoadingMore) {
            isFromLoadingMore = fromLoadingMore;
        }

        public boolean isFromLoadingMore() {
            return isFromLoadingMore;
        }
    ...
    public void loadMore(final LinearLayoutManager linearLayoutManager) {
        conversationMessagesNode.removeEventListener(conversationMessagesChildEventListener);
        //loadCapacity = loadCapacity + loadInterval;
        final Watcher w = new Watcher();
        w.setFromLoadingMore(true);
        w.setFirst(true);
        conversationMessagesChildEventListener = conversationMessagesNode.orderByChild(Message.TIMESTAMP_FIELD_KEY)
                .endAt(lastTimestamp).limitToLast(loadCapacity).addChildEventListener(new ChildEventListener() {
                    @Override
                    public void onChildAdded(DataSnapshot dataSnapshot, String prevChildKey) {
                        w.setIterator(w.getIterator() + 1);
                        Log.v(TAG, "ConversationMessagesHandler.connect.onChildAdded " + lastTimestamp);
                        Message message = new Message();
                        try {
                            message = decodeMessageSnapShop(dataSnapshot);
                            Log.d(TAG, "ConversationMessagesHandler.connect.onChildAdded.message : " + message);
                            ...
                            message.isFromLoadMore = w.isFromLoadingMore();
                            ...
                            if (w.getIterator() == loadCapacity) {
                                w.setIterator(0);
                                w.setFromLoadingMore(false);
                                linearLayoutManager.scrollToPositionWithOffset(loadCapacity - 1, 10);
                            }
                        }
                    }
...
//                Log.d(TAG, "observeMessages.onCancelled");
    ...
    private Message getLastMessage(List<Message> messages) {
        ...
        for (Message message : messages) {
            if ((new Date(message.getTimestamp())).after(new Date(oldestMessage.getTimestamp()))) {
